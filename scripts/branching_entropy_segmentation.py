...
MOTIVATION:
  ...
  forward_process_models showed the ONLY forward model that reproduces h_char uses
  ...
  But cipher_identification_tests showed word bigram MI is 4× too low for words to be
  ...
  NOBODY HAS DONE THIS. script_reverse_engineering measured corpus-wide successor
  entropy per glyph. cipher_identification_tests (part d) measured positional entropy within words.
...
import numpy as np
from common import clean_word, entropy, eva_to_glyphs, extract_words, folio_section
...
def generate_verbose_cipher(source_text, n_zones=3, expand_prob=0.8,
    ...
    Uses the positional_verbose_cipher model: consonants → 1 cipher char,
...
def main():
    pr('=' * 76)
    pr('branching_entropy_segmentation — SUB-WORD GLYPH-GROUP SEGMENTATION VIA BRANCHING ENTROPY')
    pr('=' * 76)
    pr()
    ...
    pr('─' * 76)
    pr('STEP 8: VALIDATION — KNOWN VERBOSE CIPHER (positional_verbose_cipher model)')
    pr('─' * 76)
    pr()
    pr('  Generate a verbose cipher from Italian text using the positional_verbose_cipher model.')
    ...
    # glyph_resegmentation positional classes (simplified)
    ...
    # Save results
    results_path = RESULTS_DIR / 'branching_entropy_segmentation.txt'
