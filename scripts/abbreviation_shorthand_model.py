...
- naibbe_cipher_calibration showed Naibbe verbose cipher partially matches VMS but
...
# ═══════════════════════════════════════════════════════════════════════
# FINGERPRINT FUNCTIONS (shared with naibbe_cipher_calibration)
# ═══════════════════════════════════════════════════════════════════════
...
def main():
    t0 = time.time()
    pr("=" * 75)
    pr("abbreviation_shorthand_model: MEDIEVAL ABBREVIATION/SHORTHAND MODEL vs VMS FINGERPRINT")
    ...
    pr()
    pr("  encoding_model_tournament/72 reference distances:")
    pr("    Italian (encoding_model_tournament best raw):   1.777")
    pr("    Naibbe best (naibbe_cipher_calibration):        0.3412")
    ...
    # Add naibbe_cipher_calibration references
    pr(f"  {'Naibbe best (naibbe_cipher_calibration)':>30s} {'0.341':>7s} {'0.7029':>7s} {'0.4952':>7s} "
       f"{'0.6187':>7s} {'0.3646':>7s} {'4.8456':>7s} {'0.3676':>7s} {'0.6720':>7s} {'-0.308':>7s}")
    pr(f"  {'Italian raw (encoding_model_tournament)':>30s} {'1.777':>7s} {'0.7515':>7s} {'0.6390':>7s} "
    ...
    # ── Save results ──
    out_txt = RESULTS_DIR / 'abbreviation_shorthand_model.txt'
    with open(out_txt, 'w', encoding='utf-8') as f:
        f.write(''.join(OUTPUT))
    
    out_json = RESULTS_DIR / 'abbreviation_shorthand_model_abbreviation_raw.json'
