...
MOTIVATION:
  ...
  slot_grammar_entropy_model showed the LOOP slot grammar explains only 24.2% of
  h_char depression (residual = +0.26, z = 25.9 vs NL).
  f1r_abbreviation_decode ruled out simple substitution across 6 languages.
...
def main():
    pr("=" * 80)
    pr("chunk_slot_mutual_information — INTRA-CHUNK SLOT MUTUAL INFORMATION DECOMPOSITION")
    pr("=" * 80)
    pr()
    ...
    pr(f"  VMS full h_char (slot_grammar_entropy_model): 0.6560")
    pr()
    ...
    # ══════════════════════════════════════════════════════════════════
    pr("=" * 80)
    pr("chunk_slot_mutual_information — VERDICT")
    pr("=" * 80)
    pr()
    ...
    outpath = RESULTS_DIR / "chunk_slot_mutual_information.txt"
