...
The chunk_fingerprint main script concluded "SUPPORTED" (3/4 tests passed),
...
SKEPTICISM:
  - The h_char ratio of 0.5915 computed in chunk_fingerprint includes <sp>
...
# ── Shared functions from chunk_fingerprint ────────────────────────────────────
...
def main():
    pr("=" * 76)
    pr("chunk_fingerprint_revalidation — CRITICAL REVALIDATION OF CHUNK FINGERPRINT")
    pr("=" * 76)
    pr()
    pr("  chunk_fingerprint concluded 'SUPPORTED' but the KEY test (h_ratio in NL")
    ...
    pr("─" * 76)
    pr()
    pr("  chunk_fingerprint included <sp> markers in char streams, which inflates")
    ...
    pr()
    pr(f"  Previously reported value: 0.641 (likely method C or similar)")
    ...
    pr("=" * 76)
    pr("CORRECTED VERDICT — chunk_fingerprint")
    pr("=" * 76)
    pr()
    ...
    # Save output
    out_path = RESULTS_DIR / 'chunk_fingerprint_revalidation.txt'
    ...
    json_path = RESULTS_DIR / 'chunk_fingerprint_revalidation.json'
