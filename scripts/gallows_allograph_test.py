...
QUESTION: gallows_line_position_split proved {p,f} are first-line characters and {t,k}
are body-text characters. But WHY? Two competing hypotheses:
  ...
  H2 (INDEPENDENT): p and f are separate symbols from t and k, with
      different functions (p=process, f=botanical vs
...
E. SEMANTIC RETEST — Redo gallows_semantics semantic domain analysis using ONLY
...
def main():
    pr("=" * 72)
    pr("gallows_allograph_test — THE ALLOGRAPH HYPOTHESIS: ARE p≡t AND f≡k?")
    pr("=" * 72)
    pr()
    ...
    pr("=" * 72)
    pr()
    pr("gallows_semantics found t=celestial, k=generic, f=botanical, p=process.")
    ...
    pr()
    pr("=" * 72)
    pr("gallows_allograph_test SYNTHESIS")
    pr("=" * 72)
    pr()
    ...
    # Save results
    outpath = RESULTS_DIR / 'gallows_allograph_test.txt'
