...
QUESTION:
  ...
  glyph level (word_functional_anatomy: NMI(last→next_init) = 0.080, z=313, 1.8×
...
MOTIVATION:
  ...
  The TMA paradox (mi_distance_curve) remains unresolved: VMS has 1.9× NL mean
...
CRITICAL CONFOUNDS:
  1. INVENTORY CONFOUND: F-chunks and I-chunks draw from different
     type inventories (position_identity_decomposition: J(I,F)=0.241). This means marginal
     ...
     (position_identity_decomposition: J(I,F)_syls ≈ 0.07). They are the primary baseline.

  2. LINE STRUCTURE: VMS lines are meaningful units (word_functional_anatomy: line
  ...
  4. ALPHABET SIZE: VMS has ~125 I-types and ~117 F-types (position_identity_decomposition).
...
# ═══════════════════════════════════════════════════════════════════════
# VMS CORPUS PARSING — LINE-AWARE (adapted from derivational_prefix_paradox)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# NL REF TEXT LOADING + SYLLABIFICATION (from chunk_fingerprint)
# ═══════════════════════════════════════════════════════════════════════
...
def main():
    pr("=" * 76)
    pr("cross_word_chunk_dependencies — CROSS-WORD CHUNK DEPENDENCIES")
    pr("=" * 76)
    pr()
    pr("  Does the glyph-level cross-word MI anomaly (word_functional_anatomy: NMI=0.080,")
    ...
    pr("     (position_identity_decomposition: J(I,F)=0.241). Even random pairing of words")
    ...
    if not genuine_dependency:
        ...
        pr("    The glyph-level anomaly (word_functional_anatomy: NMI=0.080) does NOT persist")
        ...
        pr("    → The glyph-level anomaly (word_functional_anatomy) was likely a grain-size artifact:")
    ...
    # Confidence updates
    pr("  REVISED CONFIDENCES (cross_word_chunk_dependencies):")
    if verdict == "CROSS_WORD_MI_NL_SYLLABLE_CONSISTENT":
        ...
        pr("  - word_functional_anatomy glyph-level anomaly: NEEDS REVALIDATION at chunk level")
    else:
    ...
    out_txt = RESULTS_DIR / 'cross_word_chunk_dependencies.txt'
    ...
    out_json = RESULTS_DIR / 'cross_word_chunk_dependencies.json'
