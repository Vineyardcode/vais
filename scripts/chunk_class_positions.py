...
APPROACH:
  1. Parse VMS into chunks → assign to 25 equivalence classes (from chunk_equivalence_classes)
...
SKEPTICISM NOTES:
  - The 25 classes are skeleton-dominated (chunk_equivalence_classes ablation: skeleton-only
  ...
  - NL syllable clustering into 25 classes may not be meaningful (chunk_equivalence_classes
  ...
  - Reference language matching by positional profile is indirect; we're
    ...
    identity.
  - The F→I MI test reuses the cross_word_chunk_dependencies framework but at class level
...
# ═══════════════════════════════════════════════════════════════════════
# VMS PARSING — LINE-AWARE (from cross_word_chunk_dependencies)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# NL TEXT LOADING & SYLLABIFICATION (from chunk_fingerprint)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# chunk_equivalence_classes EQUIVALENCE CLASS RECONSTRUCTION
# ═══════════════════════════════════════════════════════════════════════

def build_equivalence_classes(all_chunk_tokens, k=25):
    """Reproduce chunk_equivalence_classes clustering: agglomerative with JSD on
...
def main():
    ...
    pr("=" * 72)
    pr("chunk_class_positions — Positional Decomposition of the 25-Class Chunk Alphabet")
    pr("=" * 72)
    pr()
    ...
    pr("Step 2: Building 25 equivalence classes (chunk_equivalence_classes reproduction)...")
    ...
    out_json = RESULTS_DIR / 'chunk_class_positions.json'
    with open(out_json, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    pr(f"Results saved to {out_json}")

    out_txt = RESULTS_DIR / 'chunk_class_positions.txt'
