...
SKEPTICISM NOTES:
  - Phase 94 found VMS e-extension rate = 9.1% vs Latin 0.2-0.4%.
    This tests internal chunk morphology, not word endings, but signals
    fundamental structural difference.
  - Phase 33 parser defines suffixes by VMS-specific rules.  NL comparison
...
# ═══════════════════════════════════════════════════════════════════════
# VMS SUFFIX PARSER (Phase 33 SHORT list — corrected)
# ═══════════════════════════════════════════════════════════════════════
...
def collapse_echains(word):
    """Collapse runs of 'e' to single 'e' (Phase 33 convention)."""
    return re.sub(r'e{2,}', 'e', word)


def vms_parse_suffix(word):
    """Parse VMS word into (stem, suffix) using Phase 33 SHORT parser.
...
def main():
    pr("=" * 72)
    pr("Phase 104 — Do VMS Word Endings Encode Latin Inflectional Endings?")
    pr("=" * 72)
    pr()
    ...
    out_json = RESULTS_DIR / 'phase104_latin_ending_test.json'
    with open(out_json, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    pr(f"Results saved to {out_json}")

    out_txt = RESULTS_DIR / 'phase104_latin_ending_test.txt'
