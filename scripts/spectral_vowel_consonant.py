...
SKEPTICAL CONCERNS:
  ...
  - Must test whether the WORD-POSITION effect from chunk_equivalence_classes (chunks
...
import numpy as np
import random
from common import chunk_to_str, clean_word, eva_to_glyphs, extract_words_from_line, get_currier_language, load_reference_text, parse_one_chunk, parse_word_into_chunks
...
# ═══════════════════════════════════════════════════════════════════════
# EVA TOKENIZER + LOOP GRAMMAR (from chunk_fingerprint/86)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# VMS PARSING (from chunk_fingerprint/86)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# NL TEXT LOADING + SYLLABIFICATION (from chunk_fingerprint/86)
# ═══════════════════════════════════════════════════════════════════════
...
def main():
    pr("=" * 76)
    pr("spectral_vowel_consonant — VOWEL-CONSONANT SPECTRAL SEPARATION")
    pr("=" * 76)
    pr()
    ...
    pr("─" * 76)
    pr()
    pr("  chunk_equivalence_classes showed chunks cluster mainly by word position (I/M/F).")
    ...
    with open(RESULTS_DIR / 'spectral_vowel_consonant.json', 'w') as f:
        json.dump(json_out, f, indent=2, default=str)

    text_out = ''.join(OUTPUT)
    with open(RESULTS_DIR / 'spectral_vowel_consonant.txt', 'w', encoding='utf-8') as f:
        f.write(text_out)

    pr(f"  Saved to results/spectral_vowel_consonant.txt/.json")
