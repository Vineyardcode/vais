...
CONTEXT: vernacular_paragraph_initials showed vernacular recipe texts CAN produce VMS-level
...
CRITICAL SKEPTICISM:
  - paragraph_initial_analysis found 79% of paragraph-initial words are unique to that
...
import numpy as np
from common import clean_word, eva_to_glyphs, folio_section_v2 as folio_section
...
# ═══════════════════════════════════════════════════════════════════════
# EVA GLYPH PARSING (from paragraph_framing)
# ═══════════════════════════════════════════════════════════════════════
...
def synthesis(test_a, test_b, test_c, test_d, test_e, paragraphs):
    pr(f"\n{'═'*70}")
    pr(f"gallows_positional_ecology SYNTHESIS")
    pr(f"{'═'*70}")
...
def main():
    pr("═" * 70)
    pr("gallows_positional_ecology: Gallows Positional Ecology")
    pr("═" * 70)
    ...
    # Save
    txt_path = RESULTS_DIR / 'gallows_positional_ecology.txt'
