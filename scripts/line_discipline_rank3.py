#!/usr/bin/env python3
"""
Line-Discipline Rank-3 Test (portfolio S3, rung 2d) — do THREE axes
complete the discipline? (N6d)

PROVENANCE: the compression ladder so far — N6: the full measured
91-cell table + one knob forges B's line texture (D_line 6.65, bar
7.49); N6b: rank-1 fails (9.53), dominant axis = edge/paragraph; N6c:
rank-2 fails narrowly (8.12) with 96.9% variance captured and axis 2
independently confirmed as the interior residue (rho +0.80 vs N5).
This rung asks: does the THIRD component close the tournament?

MODEL M3 (fixed before fitting): deterministic rank-3 SVD
reconstruction of the double-centered log-table — three class axes and
three position profiles (~54 effective placement numbers) + the single
strength knob re-fitted on interior gain over the N6 grid and frozen.
Same declared sign convention as N6c (late bins outweigh early).

TOURNAMENT: identical machinery/bars/seeds as N6/N6b/N6c; the run
ABORTS unless B's features and the bars reproduce the committed N6
record. Entrant G1d = placement with the rank-3 table.

PRE-REGISTERED OUTCOMES:
  gate_failed            — N6 cross-checks fail to reproduce.
  still_not_compressible — G1d does not close the line group: the
                           discipline resists rank-3 compression.
  partial_bind           — line group closes, unfitted group breaks.
  three_axes_sufficient  — G1d closes BOTH groups: B's line discipline
                           compresses to three axes + one knob (~55
                           numbers vs the 91-cell table). SUGGESTIVE,
                           quarantined; axis 3's interpretation is
                           reported for the record.
OBSERVATIONAL (declared, adjudicates nothing): axis-3 profile and its
Spearman correlations with the declared predictor set and the N5 mean
ranks; axis-1/2 correlations repeated for continuity.
...
N6C_JSON = 'line_discipline_rank2.json'
...
def compress_rank3(table):
    ...
    for k in range(3):
        Ak, Vk = orient(u[:, k] * math.sqrt(sv[k]),
                        vt[k] * math.sqrt(sv[k]))
        comps.append((Ak, Vk))
        L3 = L3 + np.outer(Ak, Vk)
    P3 = np.exp(L3)
    P3 = P3 / P3.sum(axis=1, keepdims=True)
    table3 = {c: {b: float(P3[i, j]) for j, b in enumerate(BINS)}
              for i, c in enumerate(classes)}
    var3 = float((sv[:3] ** 2).sum() / (sv ** 2).sum())
    ax = {}
    for k, (Ak, Vk) in enumerate(comps, 1):
        ax[f'axis{k}'] = {c: round(float(Ak[i]), 4)
                          for i, c in enumerate(classes)}
        ax[f'profile{k}'] = {b: round(float(Vk[j]), 4)
                             for j, b in enumerate(BINS)}
    return table3, ax, var3
...
def main():
    print('=' * 76)
    print('LINE-DISCIPLINE RANK-3 TEST (S3 rung 2d) — three-axis test (N6d)')
    print('=' * 76)
    n6 = json.loads(result_path(N6_JSON).read_text(encoding='utf-8'))
    n6b = json.loads(result_path(N6B_JSON).read_text(encoding='utf-8'))
    n6c = json.loads(result_path(N6C_JSON).read_text(encoding='utf-8'))
    print(f'seed={SEED} (inherited; cross-checked vs N6/N6b/N6c)')
    ...
    table3, ax, var3 = compress_rank3(table)
    print(f'  rank-3 variance share of centered log-table: {var3:.1%}')
    for k in (1, 2, 3):
        print(f'  axis{k} (low -> high): '
              + '  '.join(f'{c} {a:+.3f}' for c, a in
                          sorted(ax[f'axis{k}'].items(),
                                 key=lambda kv: kv[1])))
    print('  profile3: ' + '  '.join(f'{b} {v:+.3f}'
                                     for b, v in ax['profile3'].items()))

    best_lam, best_err = None, 1e9
    for lam in LAMBDA_GRID:
        g = generate(b_lines, table3, keep, lam, SEED)
        err = abs(interior_gain(g, b_folios) - fB['interior_gain'])
        if err < best_err:
            best_lam, best_err = lam, err
    print(f'  LAMBDA (rank-3 table) fitted on interior gain: {best_lam} '
          f'(|err| {best_err:.4f}) — FROZEN')

    vecs = [feature_vector(generate(b_lines, table3, keep, best_lam,
                                    SEED + 200 + k), b_folios)
            for k in range(N_GEN_SEEDS)]
    g1d = {k: round(statistics.mean(v[k] for v in vecs), 4) for k in fB}
    d_line = dist(g1d, fB, LINE_GROUP)
    d_unfit = dist(g1d, fB, UNFIT_GROUP)
    print(f'  G1d (rank-3)  D_line {d_line}  D_unfitted {d_unfit}  '
          f'(full {n6["results"]["G1_discipline"]["dist"]["line"]}; '
          f'rank-1 {n6b["results"]["G1b"]["dist"]["line"]}; '
          f'rank-2 {n6c["results"]["G1c"]["dist"]["line"]})')
    ...
    common3 = sorted(set(ax['axis3']) & set(n5_means))
    corr = {'axis2_vs_n5_mean_ranks': corr_n5,
            'axis3_vs_n5_mean_ranks': spearman(
                [ax['axis3'][c] for c in common3],
                [n5_means[c] for c in common3])}
    for name, xs in preds.items():
        for k in (1, 2, 3):
            corr[f'axis{k}_vs_{name}'] = spearman(
                [ax[f'axis{k}'][c] for c in classes], xs)
    ...
    if not line_ok:
        ...
        print('    STILL NOT COMPRESSIBLE: three axes do not close the '
              'line group — the discipline resists rank-3 compression.')
    elif not unfit_ok:
        verdict = 'partial_bind'
        print('    PARTIAL (BIND): line group closes but unfitted '
              'order-texture breaks under the rank-3 table.')
    else:
        verdict = 'three_axes_sufficient'
        print('    THREE AXES SUFFICIENT: the rank-3 table closes BOTH '
              'groups — Currier B\'s line discipline compresses to '
              'three axes plus one strength knob (~55 numbers vs the '
              '91-cell table). SUGGESTIVE, quarantined; axis 3\'s '
              'interpretation is reported above for the record.')

    with open(result_path('line_discipline_rank3.json'), 'w',
        ...
        json.dump({'params': {'seed': SEED, 's7_seed': S7_SEED,
                   ...
                   'results': {'B': fB,
                               ...
                               'n6c_g1c_dist':
                                   n6c['results']['G1c']['dist'],
                               'correlations': corr},
                   'verdict': verdict},
                  fh, indent=1)
    print('\n  -> results/line_discipline_rank3.json')
