...
chunk_equivalence_classes found:  k=25 gives h_ratio=0.8486, matching NL chars (0.849).
...
import numpy as np
import random
from common import chunk_to_str, clean_word, conditional_entropy, entropy, eva_to_glyphs, extract_words_from_line, load_reference_text, parse_one_chunk, parse_word_into_chunks
...
# (identical to chunk_equivalence_classes — copied for self-containment)
# ═══════════════════════════════════════════════════════════════════════
...
def main():
    pr("=" * 76)
    pr("chunk_equivalence_revalidation — CRITICAL REVALIDATION OF CHUNK EQUIVALENCE CLASSES")
    pr("=" * 76)
    pr()
    ...
    if 25 in stability_data:
        d = stability_data[25]
        dist_h = 0.8486  # from chunk_equivalence_classes
        pr(f"  AT k=25:")
    ...
    # Load chunk_equivalence_classes cluster composition
    p86_json_path = RESULTS_DIR / 'chunk_equivalence_classes.json'
    if p86_json_path.exists():
        ...
        pr(f"  chunk_equivalence_classes JSON not found — skipping R4")
    pr()
    ...
    pr("─" * 76)
    pr()
    pr("  chunk_equivalence_classes used average-linkage. Test if results change with")
    ...
    pr("=" * 76)
    pr("CORRECTED VERDICT — chunk_equivalence_classes")
    pr("=" * 76)
    pr()
    ...
    with open(RESULTS_DIR / 'chunk_equivalence_revalidation.json', 'w') as f:
        json.dump(json_out, f, indent=2, default=str)

    text_out = ''.join(OUTPUT)
    with open(RESULTS_DIR / 'chunk_equivalence_revalidation.txt', 'w', encoding='utf-8') as f:
        f.write(text_out)

    pr(f"  Saved to results/chunk_equivalence_revalidation.txt/.json")
