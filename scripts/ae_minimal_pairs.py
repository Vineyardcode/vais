...
QUESTION:
  Is the extreme a/e inverse correlation in zodiac labels (zodiac_galenic_axis:
...
APPROACH:
  ...
  2. ZODIAC DISTRIBUTION: For minimal pairs found in zodiac labels,
     ...
     'e' variant in autumn signs (as zodiac_galenic_axis predicts).
...
# ═══════════════════════════════════════════════════════════════════════
# EVA GLYPH TOKENIZER + LOOP PARSER (from chunk_fingerprint)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# ZODIAC LABEL LOADER (from zodiac_galenic_axis)
# ═══════════════════════════════════════════════════════════════════════
...
def main():
    pr("=" * 76)
    pr("ae_minimal_pairs — a↔e MINIMAL PAIR & SUBSTITUTION ANALYSIS")
    pr("=" * 76)
    pr()
    ...
    pr("  a% pattern from zodiac_galenic_axis.")
    pr()
    ...
    if abs(r_s2a_s2e) > 0.95:
        ...
        pr("    This means the glyph-level pattern from zodiac_galenic_axis is")
    ...
    if strat_q > 0.7 and len(zodiac_ae_pairs) == 0:
        ...
        if slot2_explains:
            ...
            pr("    zodiac_galenic_axis correlation, but this could be either:")
    ...
    # Confidence updates
    pr("  REVISED CONFIDENCES (ae_minimal_pairs):")
    if mechanism == "VOCABULARY_STRATIFICATION":
        ...
        pr("  - All others unchanged from zodiac_galenic_axis")
    pr()

    # ── SAVE ──────────────────────────────────────────────────────────

    out_txt = RESULTS_DIR / 'ae_minimal_pairs.txt'
    ...
    out_json = RESULTS_DIR / 'ae_minimal_pairs.json'
