...
def test_g_compounds(paragraphs):
    ...
    pr(f"{'═'*70}")
    pr(f"""
gallows_positional_ecology found compound gallows are excluded from paragraph starts.
...
def synthesis_report(paragraphs, gal_any, total_chars):
    pr(f"\n{'═'*70}")
    pr(f"gallows_line_position_split SYNTHESIS: THE {{p,f}} vs {{t,k}} SPLIT")
    pr(f"{'═'*70}")
...
def main():
    pr("═" * 70)
    pr("gallows_line_position_split: Line-Position Gallows — The {p,f} vs {t,k} Split")
    pr("═" * 70)
    ...
    # Save
    txt_path = RESULTS_DIR / 'gallows_line_position_split.txt'
