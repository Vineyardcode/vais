...
QUESTION:
  ...
  currier_slot2_regime found Currier A leaves SLOT2 empty 68.5% vs B's 48.2%.
...
import numpy as np
import random
from common import clean_word, eva_to_glyphs
...
def main():
    pr("=" * 72)
    pr("  e_extension_morphology — SLOT2 e-Extension: Productive Morphology")
    ...
    # currier_slot2_regime predicts: extended forms (SLOT2-filled) should be enriched in B
    ...
    pr(f"  (currier_slot2_regime predicts: extended enriched in B, base enriched in A)")
    pr()
    ...
    out_path = RESULTS_DIR / 'e_extension_morphology_e_extension.json'
    with open(out_path, 'w') as f:
        json.dump(results, f, indent=2)
    pr(f"  Results saved to {out_path}")

    txt_path = RESULTS_DIR / 'e_extension_morphology_e_extension.txt'
