...
OBJECTIVE:
  ...
  (Phase 86 clusters) and repeat. Quantify how much of the VMS's
...
METHOD:
  1. Parse all VMS folios into chunk sequences (reusing Phase 85 parser)
  ...
  6. Collapse to 25 equivalence classes (Phase 86), re-compute all
...
SKEPTICISM:
  ...
  - Coverage: Phase 86 clusters only covered 57% of chunk tokens.
...
import numpy as np
import random
from common import chunk_to_str, clean_word, conditional_entropy, entropy, eva_to_glyphs, extract_words_from_line, get_currier_language, load_phase86_clusters, parse_one_chunk, parse_word_into_chunks
...
def run_analysis():
    pr("=" * 72)
    pr("PHASE 98 — GENERATIVE CHUNK MODEL")
    pr("=" * 72)
    pr()
    ...
    pr()
    pr("─" * 72)
    pr("STEP 7: Collapse to 25 equivalence classes (Phase 86)")
    pr("─" * 72)
    ...
    pr(f"\n  Phase 86 mapped: {mapped_count} / {len(all_chunk_types)} chunk types")
    ...
    json_path = RESULTS_DIR / 'phase98_generative_chunk_model.json'
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(save_data, f, indent=2, default=str)
    pr(f"\n  Results saved to {json_path}")

    txt_path = RESULTS_DIR / 'phase98_generative_chunk_model.txt'
