...
# ═══════════════════════════════════════════════════════════════════════
# VMS TEXT EXTRACTION (CORRECTED from currier_dichotomy_resolution)
# ═══════════════════════════════════════════════════════════════════════



def get_currier_language_from_header(filepath):
    """Parse $L= tag from IVTFF folio header (corrected method from currier_dichotomy_resolution)."""
...
def run_analysis():
    pr("=" * 72)
    pr("historical_validation — HISTORICAL VALIDATION")
    pr("=" * 72)
    pr()
    ...
    # Use rank-normalized top-N comparison (matching currier_dichotomy_resolution method)
    ...
    evidence = [
        ...
        ('chunk_alphabet_decipherment: Latin best CV fit (16.7%)',
         {'Latin': +1, 'Italian': +1, 'German': 0, 'Czech': -1, 'Constructed': 0}),
        ('currier_dichotomy_resolution: Same grammar, different vocabulary (A/B)',
         {'Pharmaceutical': +1, 'Alchemical': +1, 'Astronomical': 0, 'Encyclopedic': +1}),
        ('generative_chunk_model: h_ratio = 0.818 (NL-like)',
         {'Meaningless': -2, 'Latin': +1, 'Italian': +1, 'German': +1,
          'Czech': +1, 'Constructed': +1}),
        ('latin_paragraph_initials: Gallows ≠ letter substitution',
         {'Simple substitution': -2, 'Verbose cipher': +1,
          'Abbreviation+cipher': +1, 'Constructed script': +1}),
        ('script_typology_comparison: VMS is typological outlier (p=1.0)',
         {'Simple substitution': -1, 'Verbose cipher': 0,
          'Constructed script': +2, 'Meaningless': -1}),
        ('chunk_equivalence_classes: 25-class chunk equivalence (structured)',
        ...
        ('vernacular_paragraph_initials: Recipe texts match gallows coverage',
         {'Pharmaceutical': +1, 'Alchemical': +1, 'Astronomical': -1, 'Encyclopedic': 0}),
        ('4/6 pharma features match',
    ...
    logical_checks = [
        ...
        ("Simple substitution rejected by latin_paragraph_initials + 100",
    ...
    facts_95 = [
        ...
        ("Simple substitution cipher is ruled out", "99%",
         "latin_paragraph_initials gallows, chunk_alphabet_decipherment decipherment failure"),
    ...
    prob_findings = [
        ("Substrate language is Italian or Latin", "80%",
         "chunk_alphabet_decipherment/101 NL fit; Italian provenance; vernacular_paragraph_initials coverage"),
        ...
        ("A/B is register variation within one system", "80%",
         "currier_dichotomy_resolution: shared grammar, different vocabulary"),
        ("Encoding uses complex/verbose cipher or constructed script", "75%",
         "latin_paragraph_initials gallows, chunk_fingerprint Pelling, script_typology_comparison typology"),
        ...
        ("A/B partly confounded with manuscript section", "85%",
         "currier_dichotomy_resolution: within-section JSD ≈ between-section JSD"),
        ("VMS chunks resemble abugida sub-units", "70%",
         "script_typology_comparison: closest typological category, but only on h_ratio"),
    ]
    ...
    pr("    5. Run-to-run instability (chunk_alphabet_decipherment) means stochastic methods")
    ...
    # SAVE RESULTS
    # ───────────────────────────────────────────────────────────────────
    json_path = RESULTS_DIR / 'historical_validation.json'
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)

    txt_path = RESULTS_DIR / 'historical_validation.txt'
