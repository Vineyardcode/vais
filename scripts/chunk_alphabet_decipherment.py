...
OBJECTIVE:
  Treat the 25 equivalence classes (chunk_equivalence_classes) as a 25-symbol alphabet.
...
METHOD:
  1. Collapse all VMS words to 25-class sequences using the
     nearest-neighbour extended mapping (generative_chunk_model).
...
import numpy as np
import random
from common import chunk_to_str, clean_word, eva_to_glyphs, extract_words_from_line, get_currier_language, load_chunk_equivalence_clusters, parse_one_chunk
...
# ═══════════════════════════════════════════════════════════════════════
# EVA GLYPH TOKENIZER (from chunk_fingerprint/98)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# MAURO'S LOOP GRAMMAR — CHUNK PARSER (from chunk_fingerprint/98)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# chunk_equivalence_classes CLUSTER MAPPING (from generative_chunk_model)
# ═══════════════════════════════════════════════════════════════════════
...
def run_analysis():
    pr("=" * 72)
    pr("chunk_alphabet_decipherment — TARGETED DECIPHERMENT WITH 25-CLASS ALPHABET")
    pr("=" * 72)
    pr()
...
""")
    ...
    json_path = RESULTS_DIR / 'chunk_alphabet_decipherment.json'
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(save_data, f, indent=2, default=str)
    pr(f"\n  Results saved to {json_path}")

    txt_path = RESULTS_DIR / 'chunk_alphabet_decipherment.txt'
