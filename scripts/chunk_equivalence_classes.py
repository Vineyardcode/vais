...
MOTIVATION:
  ...
  chunk_fingerprint found 523 chunk types — too many for an alphabet (~30)
...
# ═══════════════════════════════════════════════════════════════════════
# VMS PARSING (from chunk_fingerprint)
# ═══════════════════════════════════════════════════════════════════════
...
def main():
    pr("=" * 76)
    pr("chunk_equivalence_classes — CHUNK EQUIVALENCE CLASS DISCOVERY")
    pr("=" * 76)
    pr()
    ...
    with open(RESULTS_DIR / 'chunk_equivalence_classes.json', 'w') as f:
        json.dump(json_out, f, indent=2, default=str)

    text_out = ''.join(OUTPUT)
    with open(RESULTS_DIR / 'chunk_equivalence_classes.txt', 'w', encoding='utf-8') as f:
        f.write(text_out)

    pr()
    pr(f"  Results saved to results/chunk_equivalence_classes.txt/.json")
